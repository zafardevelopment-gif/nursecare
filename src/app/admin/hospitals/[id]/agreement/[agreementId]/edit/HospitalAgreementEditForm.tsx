...
export default function HospitalAgreementEditForm({
  agreement,
  hospitalId,
}: {
  agreement: Agreement
  ...
  const [paymentType, setPaymentType] = useState<PaymentType>(agreement.payment_type as PaymentType)
  const [missedAction, setMissedAction] = useState<MissedAction>(
    (agreement.daily_missed_action || agreement.weekly_missed_action || agreement.monthly_missed_action || 'pause') as MissedAction
  )
  const [reminderHrs, setReminderHrs] = useState<number[]>(agreement.reminder_hours ?? [48, 24, 6])
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTx(async () => {
      const res = await updateHospitalAgreementAction(fd)
    ...
    <form onSubmit={handleSubmit}>
        ...
        <div style={{ padding: '1.25rem 1.5rem' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
            ...
            <div className="form-group">
              ...
              <input type="date" name="end_date" required className="form-input" defaultValue={agreement.end_date} />
            </div>
          </div>
          <div className="form-group">
        ...
        <div style={{ padding: '1.25rem 1.5rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 20 }}>
            {([
              ...
              <div key={pt.key} onClick={() => setPaymentType(pt.key)} style={{
                ...
                {paymentType === pt.key && (
                  <div style={{ position: 'absolute', top: 7, right: 8, width: 18, height: 18, background: 'var(--teal)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.6rem', color: '#fff', fontWeight: 800 }}>✓</div>
                )}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                <div className="form-group">
                  ...
                  <div style={{ display: 'flex', border: '1.5px solid var(--border)', borderRadius: 8, overflow: 'hidden' }}>
                    <input type="number" name="adv_deadline_hrs" defaultValue={agreement.adv_deadline_hrs ?? 6} min={1} style={{ flex: 1, border: 'none', outline: 'none', padding: '8px 12px', fontSize: '0.88rem', fontFamily: 'inherit' }} />
                    <span style={suffixStyle}>hrs before</span>
                  </div>
                </div>
                <div className="form-group">
                  <label className="form-label">On Non-Payment</label>
                  <input className="form-input" value="Auto-cancel (locked)" readOnly style={{ background: 'var(--cream)', color: 'var(--muted)', cursor: 'not-allowed' }} />
                </div>
              </div>
            </div>
          )}
            ...
            <div style={{ background: 'rgba(14,123,140,0.04)', border: '1px solid rgba(14,123,140,0.14)', borderRadius: 10, padding: 18 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1rem' }}>
                <div className="form-group">
                  ...
                  <div style={{ display: 'flex', border: '1.5px solid var(--border)', borderRadius: 8, overflow: 'hidden' }}>
                    <input type="number" name="daily_deadline_hrs" defaultValue={agreement.daily_deadline_hrs ?? 24} min={1} style={{ flex: 1, border: 'none', outline: 'none', padding: '8px 12px', fontSize: '0.88rem', fontFamily: 'inherit' }} />
                    <span style={suffixStyle}>hrs before</span>
                  </div>
                </div>
                <div className="form-group">
                  ...
                  <div style={{ display: 'flex', border: '1.5px solid var(--border)', borderRadius: 8, overflow: 'hidden' }}>
                    <input type="number" name="daily_grace_hrs" defaultValue={agreement.daily_grace_hrs ?? 2} min={0} style={{ flex: 1, border: 'none', outline: 'none', padding: '8px 12px', fontSize: '0.88rem', fontFamily: 'inherit' }} />
                ...
                <div className="form-group">
                  ...
                  <div style={{ display: 'flex', border: '1.5px solid var(--border)', borderRadius: 8, overflow: 'hidden' }}>
                    <input type="number" name="daily_cancel_misses" defaultValue={agreement.daily_cancel_misses ?? 2} min={1} style={{ flex: 1, border: 'none', outline: 'none', padding: '8px 12px', fontSize: '0.88rem', fontFamily: 'inherit' }} />
            ...
            <div style={{ background: 'rgba(14,123,140,0.04)', border: '1px solid rgba(14,123,140,0.14)', borderRadius: 10, padding: 18 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1rem' }}>
                <div className="form-group">
                  ...
                  <select name="weekly_payment_day" className="form-input" defaultValue={agreement.weekly_payment_day ?? 'monday'}>
                ...
                <div className="form-group">
                  <label className="form-label">Deadline</label>
                  <div style={{ display: 'flex', border: '1.5px solid var(--border)', borderRadius: 8, overflow: 'hidden' }}>
                    <input type="number" name="weekly_deadline_hrs" defaultValue={agreement.weekly_deadline_hrs ?? 72} min={1} style={{ flex: 1, border: 'none', outline: 'none', padding: '8px 12px', fontSize: '0.88rem', fontFamily: 'inherit' }} />
                    <span style={suffixStyle}>hrs before</span>
                  </div>
                </div>
                <div className="form-group">
                  <label className="form-label">Grace Period</label>
                  <div style={{ display: 'flex', border: '1.5px solid var(--border)', borderRadius: 8, overflow: 'hidden' }}>
                    <input type="number" name="weekly_grace_hrs" defaultValue={agreement.weekly_grace_hrs ?? 6} min={0} style={{ flex: 1, border: 'none', outline: 'none', padding: '8px 12px', fontSize: '0.88rem', fontFamily: 'inherit' }} />
                    <span style={suffixStyle}>hrs</span>
                  </div>
                </div>
              </div>
            ...
            <div style={{ background: 'rgba(14,123,140,0.04)', border: '1px solid rgba(14,123,140,0.14)', borderRadius: 10, padding: 18 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1rem' }}>
                <div className="form-group">
                  ...
                  <div style={{ display: 'flex', border: '1.5px solid var(--border)', borderRadius: 8, overflow: 'hidden' }}>
                    <input type="number" name="monthly_billing_day" defaultValue={agreement.monthly_billing_day ?? 25} min={1} max={28} style={{ flex: 1, border: 'none', outline: 'none', padding: '8px 12px', fontSize: '0.88rem', fontFamily: 'inherit' }} />
                ...
                <div className="form-group">
                  ...
                  <div style={{ display: 'flex', border: '1.5px solid var(--border)', borderRadius: 8, overflow: 'hidden' }}>
                    <input type="number" name="monthly_advance_days" defaultValue={agreement.monthly_advance_days ?? 15} min={0} style={{ flex: 1, border: 'none', outline: 'none', padding: '8px 12px', fontSize: '0.88rem', fontFamily: 'inherit' }} />
                ...
                <div className="form-group">
                  <label className="form-label">Grace Period</label>
                  <div style={{ display: 'flex', border: '1.5px solid var(--border)', borderRadius: 8, overflow: 'hidden' }}>
                    <input type="number" name="monthly_grace_hrs" defaultValue={agreement.monthly_grace_hrs ?? 24} min={0} style={{ flex: 1, border: 'none', outline: 'none', padding: '8px 12px', fontSize: '0.88rem', fontFamily: 'inherit' }} />
                    <span style={suffixStyle}>hrs</span>
                  </div>
                </div>
              </div>
          ...
          <div style={{ marginTop: 22, paddingTop: 18, borderTop: '1px solid var(--border)' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 8 }}>
              {reminderHrs.map(h => (
                <span key={h} onClick={() => removeReminder(h)} style={{
              ...
              <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                <input type="number" value={newReminder} onChange={e => setNewReminder(e.target.value)} placeholder="hrs"
      ...
      <div style={{
        ...
        <div style={{ fontSize: '0.78rem', color: 'var(--muted)' }}>
          ℹ️ Saving resets agreement to Draft status — you'll need to re-approve.
        </div>
        <div style={{ display: 'flex', gap: 10 }}>
          <a href={`/admin/hospitals/${hospitalId}/agreement/${agreement.id}`}
            style={{ background: 'var(--cream)', color: 'var(--ink)', border: '1px solid var(--border)', padding: '9px 18px', borderRadius: 9, fontWeight: 600, fontSize: '0.85rem', textDecoration: 'none', display: 'inline-flex', alignItems: 'center' }}>
            Cancel
          </a>
          <button type="submit" disabled={isPending}
            style={{ background: 'var(--teal)', color: '#fff', border: 'none', padding: '9px 22px', borderRadius: 9, fontWeight: 700, fontSize: '0.85rem', cursor: isPending ? 'not-allowed' : 'pointer', fontFamily: 'inherit', opacity: isPending ? 0.7 : 1 }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginTop: 6 }}>
        {(['pause', 'cancel'] as const).map(opt => (
          <div key={opt} onClick={() => onChange(opt)} style={{
            ...
            <div style={{ width: 13, height: 13, borderRadius: '50%', border: `2px solid ${value === opt ? 'var(--teal)' : 'var(--border)'}`, background: value === opt ? 'var(--teal)' : 'transparent', flexShrink: 0 }} />
...
const suffixStyle: React.CSSProperties = {
  padding: '0 10px', background: 'var(--cream)', fontSize: '0.75rem', fontWeight: 600,
