...
import { approveAgreementAsHospital } from '../actions'
...
export default function HospitalAgreementApproveClient({ agreement }: { agreement: Agreement }) {
  const [isPending, startTransition] = useTransition()
  const [err, setErr] = useState<string | null>(null)
  const [confirmed, setConfirmed] = useState(false)
  const router = useRouter()

  const alreadyApproved = !!agreement.hospital_approved_at

  function handleApprove() {
    const fd = new FormData()
    fd.set('agreement_id', agreement.id)
    startTransition(async () => {
      const res = await approveAgreementAsHospital(fd)
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: '1.5rem', alignItems: 'start' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        ...
        {/* Nurse approval status */}
        <div className="dash-card">
          <div className="dash-card-header"><span className="dash-card-title">Nurse Approval</span></div>
          <div className="dash-card-body">
            {agreement.nurse_approved_at ? (
              <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                <div style={{ color: '#27A869', fontSize: '1.2rem' }}>✓</div>
                <div>
                  <div style={{ fontSize: '0.82rem', fontWeight: 700, color: '#1A7A4A' }}>Nurse Approved</div>
                  <div style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>{new Date(agreement.nurse_approved_at).toLocaleString()}</div>
                </div>
              </div>
            ) : (
              <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                <div style={{ color: '#F5842A', fontSize: '1.2rem' }}>⏳</div>
                <div style={{ fontSize: '0.82rem', color: 'var(--muted)' }}>Awaiting nurse approval</div>
              </div>
            )}
          </div>
        </div>
