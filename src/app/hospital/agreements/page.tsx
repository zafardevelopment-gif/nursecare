...
const STATUS_LABELS: Record<string, { label: string; color: string; bg: string }> = {
  ...
  hospital_approved: { label: 'You Approved',        color: '#0E7B8C', bg: '#E8F4FD' },
...
export default async function HospitalAgreementsPage() {
  ...
  const { data: agreements } = await supabase
    .from('agreements')
    .select('id, title, status, generated_at, nurse_id, template_version, nurse_approved_at, hospital_approved_at')
    .eq('hospital_id', user.id)
    .order('generated_at', { ascending: false })

  const pendingCount = (agreements ?? []).filter(a => !a.hospital_approved_at).length
    ...
    <div className="dash-shell">
      <div className="dash-header">
        <div>
          ...
          <p className="dash-sub">Review and approve agreements with healthcare providers</p>
        </div>
        {pendingCount > 0 && (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid var(--border)' }}>
                  {['Title', 'Status', 'Nurse Approval', 'Your Approval', 'Date', ''].map(h => (
                ...
                {(agreements ?? []).map(a => {
                  ...
                  return (
                    <tr key={a.id} style={{ borderBottom: '1px solid var(--border)' }}>
                      ...
                      <td style={{ padding: '12px 16px' }}>
                        <Link href={`/hospital/agreements/${a.id}`} style={{
                          background: a.hospital_approved_at ? 'var(--cream)' : '#0E7B8C',
                          color: a.hospital_approved_at ? 'var(--ink)' : '#fff',
                          border: a.hospital_approved_at ? '1px solid var(--border)' : 'none',
                          padding: '6px 14px', borderRadius: 7, fontSize: '0.78rem',
                          fontWeight: 700, textDecoration: 'none',
                        }}>
                          {a.hospital_approved_at ? 'View' : 'Review & Approve →'}
                        </Link>
                      </td>
                    </tr>
                  )
